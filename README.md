...
#### 步骤 5：8-邻域空间桶搜索与运动学触足建边 (Kinematic Reachable Edge Building)
* **目的**：判定相邻两个空间踏面节点之间，四足机器人能否安全迈出一步。
* **机制**：
  遍历节点 $u$，在周围 8 个相邻柱中寻找候选节点 $v$，校验四足运动学物理极限：
  1. 垂直抬腿高差：$|u.z - v.z| \le \text{max\_step\_height}$（例如 $0.22\text{m}$）
  2. 水平跨步步长：$\sqrt{\Delta x^2 + \Delta y^2} \le \text{max\_stride\_length}$（例如 $0.35\text{m}$）
  3. 净空无顶头：$u$ 与 $v$ 均满足通行高度
  通过校验即建立双向拓扑边，并计算平滑势能代价：$\text{cost} = \sqrt{\Delta xy^2 + \Delta z^2} + 2.0 \cdot \Delta z$。
* **效果**：不论台阶视角是 50°、60° 还是 70°，只要在抬腿与步长极限内，拓扑边全部自然连通；而悬崖、过高断坎与超宽沟壑被自然隔断。
...
| `max_step_height` | `0.22` | 四足机器人单步最大踏步高差极限 (m) |
